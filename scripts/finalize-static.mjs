...
const manifest = JSON.parse(
  await readFile(
    new URL('../dist/server/vinext-prerender.json', import.meta.url),
    'utf8',
  ),
);
for (const route of ['/', '/japan', '/japan/fukuoka']) {
  if (
...
for (const route of ['japan', 'japan/fukuoka']) {
  ...
  await copyFile(
    new URL(route + '.html', root),
    new URL(route + '/index.html', root),
  );
}
const photos = JSON.parse(
  await readFile(
    new URL('../japan/fukuoka/lib/place-photos.json', import.meta.url),
    'utf8',
  ),
);
...
for (const page of [
  ...
  for (const [, href] of html.matchAll(/(?:src|href)="(\/[^"]*)"/g)) {
    ...
    if ((await stat(target)).isDirectory()) {
      target = new URL(
        (local ? local.replace(/\/?$/, '/') : '') + 'index.html',
        root,
      );
    }
    await stat(target);
...
console.log(
  `Static pages ready: /, /japan/ and /japan/fukuoka/ (${Object.keys(photos).length} place photos)`,
);
