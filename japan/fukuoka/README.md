...
本目录保存原旅行网站的页面、固定导航、每日地图、酒店和美食地图、21 张实景照片及来源。网站通过根目录 `app/japan/fukuoka/` 挂载到 `/japan/fukuoka/`；在仓库根目录运行 `npm ci` 和 `npm run dev`，无需在这里单独安装依赖。
...
- `public/`：照片和图标源文件，启动/构建时同步到根目录的 `public/japan/fukuoka/`。
